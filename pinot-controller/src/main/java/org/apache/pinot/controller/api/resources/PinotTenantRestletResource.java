...
public class PinotTenantRestletResource {
  ...
  public String getTablesOnTenant(
    ...
    if (tenantType == null || tenantType.isEmpty() || tenantType.equalsIgnoreCase("server")) {
      return getTablesServedFromServerTenant(tenantName, headers.getHeaderString(Constants.DATABASE));
    } else if (tenantType.equalsIgnoreCase("broker")) {
      return getTablesServedFromBrokerTenant(tenantName, headers.getHeaderString(Constants.DATABASE));
    } else {
